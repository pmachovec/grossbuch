...
Package managers are typical for~\hyperref[unixlinux]{Linux} \hyperref[os]{operating systems}, for~example Ubuntu or~Debian use the~popular~Apt. Oracle Linux has the~\href{https://rpm-software-management.github.io/}{DNF} (formerly~Yum). But~there~are also some package managers available for~Windows, like \href{https://www.nuget.org/}{NuGet} or~\href{https://chocolatey.org/}{Chocolatey}. There~is also the~PackageManagement (formerly OneGet), which connects more Windows package managers to~one application. Some people say that the~PackageManagement is a~package manager manager.
