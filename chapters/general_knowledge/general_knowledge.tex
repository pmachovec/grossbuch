...
\begin{itemize}
    ...
    \itembf{Character encoding}, also called \itq{encoding scheme} or~simply \itq{encoding}, is~a~way how code points (numbers) from a~character set are~stored in~\hyperref[bitsandbytes]{bytes} -- it~describes how~characters are~\hyperref[encoding]{encoded} to~bytes.
...
It~always produces a~hash of~160 \hyperref[bitsandbytes]{bits}.
As~the~very first version it~has been already broken and~shouldn't be used any~more.

\mbox{The SHA-2} algorithm has~many subversions, each producing a~hash of~different \hyperref[bitsandbytes]{bit} length.
This forms another confusion -- the~length is specified \textbf{instead} of the~main version.
I.e.,~when you see for~example \mbitq{SHA-256}, it~means \mbox{the SHA-2} version producing a~hash of~256 \hyperref[bitsandbytes]{bits}.
...
It~also has subversions creating hashes of~different \hyperref[bitsandbytes]{bit} lengths.
...
For~example, \mbox{a SHA-3} algorithm producing \mbox{a 256-\hyperref[bitsandbytes]{bit}} hash is~denoted \mbox{as \itq{SHA3-256}.}
