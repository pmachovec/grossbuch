...
\noindent The~typical \hyperref[shebang]{shebang} \textitquoted{\#!/bin/sh} is~a~symbolic link to~an~SH language of~a~specific Unix system.
...
The~\textitquoted{apt} command serves for~handling packages of~APT.

\newsubsection{Apt VS Apt-get}
The~\textitquoted{apt-get} is~a~predecessor of~the~\textitquoted{apt} command.
There's a~possibility of~caching \textitquoted{apt-get} commands, but~it~requires user's active participation.
The~newer \textitquoted{apt} command handles caching automatically.

\newsection{Bashrc VS Bash Profile}
\index{Bashrc}
\index{Bash profile}
Both these scripts are a~sort of~initial scripts.
The~\textitquoted{.bashrc} script is~executed any time when a~new terminal window is~opened.
The~\textitquoted{.bash\_profile} script is~executed after an~user having the~script in~his home folder logins to~the~system, even remotely.

Generally, to~reload the~\textitquoted{.bashrc} script you must open a~new terminal window.
To~reload the~\textitquoted{.bash\_profile} script you~must reboot the~system.
However, this can~be substituted by~giving the~path to~the~script to~the~\textitquoted{source} command, i.e.,~by~running \textitquoted{source \textasciitilde/.bashrc} or~\textitquoted{source \textasciitilde/.bash\_profile}.
