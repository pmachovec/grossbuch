...
Last line holds the~return value, the~\mbit{return} keyword isn't used.
...
\example[parameter type inference]
%! language = TEXT
\begin{lstlisting}[language=Kotlin]
...
\example[lambda function with receiver type]
%! language = TEXT
\begin{lstlisting}[language=Kotlin]
...
The~parameter is accessible in~the~commands section under the keyword~\mbitq{it}.
...
You~may~have \mbitq{if--then--else} branching in~a~lambda function and~you~may want to~exit the~function at~a~certain point before reaching the~end.
And~because lambda functions don't~support the~standard \mbitq{return} statement, it~isn't so~straightforward.
...
Then, inside the~lambda function body, the~call \mbitqls{return@LABEL\_NAME;;;RETURN\_VALUE}, or~\mbitq{return@LABEL\_NAME} without any~return value (returning \hyperref[kotlinunit]{\mbit{Unit}}), must~be made.
