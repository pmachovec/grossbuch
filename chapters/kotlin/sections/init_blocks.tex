...
They can~access fields of~their holding structures (classes or~objects), but~if~these aren't initialized with the~holding structure (which can~happen for~\textit{lateinit} fields), it~thows the~exception \itq{UninitializedPropertyAccessException}.

There can~be more initializer blocks in~one class or~object.
In~that case they're executed in~the~top--down order of~their definition.
...
Initializer blocks can~access arguments of~\hyperref[kotlinprimaryconstructor]{primary constructors}.
To~read an~argument of~a~standard constructor, the~only way is to~pass it somehow to~the~\hyperref[kotlinprimaryconstructor]{primary~one}.
