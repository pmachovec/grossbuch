\newsection{Operators}
\index{Operator}
\label{kotlinoperator}
An~operator is one or~more symbols, can~be even characters (like \itq{in} or~\mbitq{by}), that either somehow changes a~value of~an~entity (unary operator) or~somehow combines values of~two entities (binary operator).
...
\newsubsection{Operators Overloading}
\index{Operators overloading}
\label{kotlinoperatoroverload}
Each~operator, including \hyperref[kotlinindexedaccess]{\textit{indexed access}} and~\mbit{\hyperref[kotlininvoke]{invoke}}[,] has~an~underlying function with~a~name, which~is~executed when~the~operator is~used.
...
For~\hyperref[kotlinindexedaccess]{\textit{indexed access}} and~\mbit{\hyperref[kotlininvoke]{invoke}} operators, however, this~isn't true.
