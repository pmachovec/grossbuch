...
You~need to~specify at~least \hyperref[kotlingetset]{getter} (for~immutable property with the~\mbitq{val} keyword).
Furthermore, \mbox{the \hyperref[kotlingetset]{\itq{field}}} keyword isn't available in~such getters and~setters.

If~you want the~property to~be mutable (with the~\mbitq{var} keyword), it's~even more complicated.
...
That means that special classes with the~\mbitq{--Kt} suffix in~names and~elaborated combination of~static methods are~generated for~them during compilation.
