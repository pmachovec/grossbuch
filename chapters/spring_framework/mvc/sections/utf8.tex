\newsection{Non--ASCII characters}
As~many other applications, Spring~MVC doesn't like non--ASCII characters by~default.
To~make it properly handling all the~\hyperref[charactersetencoding]{Unicode} characters you~must add the~character \hyperref[utf]{encoding} property element into the~template resolver and~Thymeleaf view resolver bean configurations (shown in~an~example above) and~also configure a~filter in~the~\mbit{web.xml} file.
