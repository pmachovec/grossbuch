...
The~\mbit{properties} file is~referenced in~the~element \mbit{property--placeholder}, attribute \textit{location}, from the~\hyperref[namespace]{namespace} \textit{\href{http://www.springframework.org/schema/context}{http://www.springframework.org/schema/context}}.
...
First make sure the~file is~saved in~that format, some IDEs and~editors tend to~preserve \mbox{the ISO~8859--1} for~\textit{properties} files regardless corresponding project settings.
