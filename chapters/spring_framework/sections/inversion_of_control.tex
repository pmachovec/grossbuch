...
This is done by~the~element \mboxtextit{component-scan}, attribute \mboxtextit{base-package}, from the~\hyperref[namespaces]{namespace} \mboxtextitquoted{\href{http://www.springframework.org/schema/context}{http://www.springframework.org/schema/context}}.
The~convention is to~call the namespace as \mboxtextit{context} in the~XML file.
Bean classes contain bean~IDs as~a~parameter of~the~class annotation \mboxtextit{@Component}.
The~annotation is processed in~runtime, therefore, if~you~make a~mistake in~it, you~won't find~out during application build.

Component scanning works even for~all~subpackages of~the~package specified in~the~\mboxtextit{component-scan} element.
\newpage
...
The~class reference is then given to~the~constructor of~the~\hyperref[springcontainrer]{Spring container} implementation \mboxtextit{AnnotationConfigApplicationContext} (instead of~the~XML file given to~the~\mboxtextit{ClassPathXmlApplicationContext} constructor).
