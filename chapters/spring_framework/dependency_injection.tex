...
\warning Dependency fields in~the~main bean class can't be declared as~final when using the~setter injection. Final fields must be \hyperref[definitiondeclaration]{defined} no~later than in~the~constructor, and~that can't happen with the~setter injection.
...
\begin{tikzpicture}[remember picture, overlay]
    ...
    \drawarrow{di2xml1depfield}{di2java2depfield}
    ...
    \drawarrow{di2java2interface.south}{di2java3interface}[green][.3][bend right=12mm]
...
Similarly as for~the~\hyperref[inversionofcontrol]{inversion of~control} even for~the~\hyperref[dependencyinjection]{dependency injection} the~configuration XML file can~be completely omitted and~replaced by a~special configuration class with the~\textit{Configuration} annotation.
