...
Installed package are~available as~standard \hyperref[nodejsmodule]{modules} by~their names.
...
\example[loading package with \hyperref[amd]{\textit{requirejs}}]
\begin{lstlisting}[language=JavaScript]
...
To~create a~new~profile run \mbitqls{npmrc -c;;;PROFILE\_NAME}. To~switch to~a~different profile run \mbitqls{npmrc;;;PROFILE\_NAME}.
...
The~\mbit{node\_modules} doesn't have to~be~included. When the~project is~installed by~NPM, all~dependencies will~be~downloaded automatically.
The~\mbit{package.json} file must contain package name, three--number version and~the~path to~the~main \hyperref[nodejsmodule]{Node.js module} file.
