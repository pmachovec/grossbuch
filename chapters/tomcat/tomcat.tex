...
\hyperref[jsp]{JSP}~and~HTML pages can~be located anywhere in~the~application folder, in~a~web browser you~access them by~an~address in~the~pattern \mboxtextitquoted{HOST:PORT/APPLICATION\_FOLDER/REST\_OF\_PATH}.
If~the~application contains some Java code (e.g.,~\hyperref[servlet]{servlets}), its~\textit{class} files must~be located in~\mboxtextitquoted{APPLICATION\_FOLDER/WEB-INF/classes}.
If~the~application uses some third party libraries (e.g.,~\hyperref[jstl]{JSTL}; yes,~Tomcat doesn't understand JSTL by~default), their jars must~be located in~\mboxtextitquoted{APPLICATION\_FOLDER/WEB-INF/lib}.
