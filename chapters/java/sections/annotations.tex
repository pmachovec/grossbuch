...
\warning Although annotation \hyperref[declarationdefinition]{definitions} are much~closer to~interfaces (\mbitq{interface} in~the~keyword, methods without bodies, \textbf{extending} \mbox{the \itq{Annotation}} interface), those structures are still denoted as~annotation \textbf{classes}.
...
If~an~annotation contains only one \hyperref[parameterargument]{parameter} (method) called \mbitq{value}, it~isn't necessary to~specify the~parameter name when setting~it.
