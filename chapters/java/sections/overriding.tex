...
When the~compiler encounters a~method with this annotation, it~checks that the~declaration of~the~method in~the~class matches the~definition in~an~interface or~a~superclass and~if~not, it~stops the~compilation with complaining that \itq{method does not override or implement a method from a supertype}.
...
\example[overriding without the~annotation]
%! language = TEXT
\begin{lstlisting}[language=Java]
    public class Parent {
        public void doSomething() {
...
\example[this will be compiled, subtypes of~return values are allowed]
%! language = TEXT
\begin{lstlisting}[language=Java]
    public class Parent {
...
\example[works even with return value subtypes]
%! language = TEXT
\begin{lstlisting}[language=Java]
    public class Parent {
