...
At~the~end define a~handler for~each definition inside \mbit{module.exports} object.
...
To~load the~module in~a~different JavaScript file use the~\mbit{require} function with the~path to~the~module (without the~\itq{js} extension) as~the~argument and~assign the~result to~a~variable.
...
\example{CommonJS module definition and~usage}
\emtwen
%! language = TEXT
